...
// ── Wave Firebase project ID — used for ID token verification ────────────────
const WAVE_PROJECT_ID = 'shadow-nexus-wave';
...
const ALLOWED_ORIGINS = [
  ...
  'https://waveshadownexussocial.online',
  'http://localhost',
  'http://127.0.0.1'
];
...
//
// Verifies a Firebase ID token using Google's public keys and returns the
// decoded payload (including uid) on success, or null on failure.
// This is a lightweight RS256 JWT verifier — no Admin SDK required in a Worker.
//
// Usage:
//   const claims = await verifyFirebaseIdToken(idToken, firebaseProjectId);
//   if (!claims || claims.sub !== expectedUid) { /* reject */ }
//
async function verifyFirebaseIdToken(idToken, projectId) {
  try {
    ...
    // ── 6. Decode and validate payload claims ──
    const payload = JSON.parse(atob(parts[1].replace(/-/g,'+').replace(/_/g,'/')));
    const now = Math.floor(Date.now() / 1000);
    if (payload.exp < now)               return null;  // expired
    if (payload.iat > now + 300)         return null;  // issued far in the future
    if (payload.aud !== projectId)       return null;  // wrong project
    if (!payload.sub)                    return null;  // no UID
    if (payload.firebase?.sign_in_provider === 'anonymous') return null; // block anonymous
...
async function handleMpuCreate(request, env, cors, sec) {
  ...
  // ── Verify caller identity — token uid must match the upload uid ──────────
  // This prevents one user from uploading files into another user's storage path.
  const claims = await verifyFirebaseIdToken(idToken, WAVE_PROJECT_ID);
  if (!claims || claims.sub !== uid) {
    return new Response(JSON.stringify({ error: 'Unauthorized — valid Firebase ID token required' }), {
...
async function handleSocialBridge(request, env, cors, sec) {
  ...
  let socialIdToken, socialLocalId, socialEmail, socialProfile;
  try {
  ...
  // ── Step 2: Verify the Social ID token and extract Auth-level profile fields ─
  ...
  // The response also contains the Auth-level profile: displayName and photoUrl.
  try {
    ...
    // Extract Auth-level profile fields from the verified account record.
    // photoUrl in the Identity Toolkit response uses a lowercase 'l'.
    const authUser = verifyData.users[0];
    socialProfile = {
      displayName: authUser.displayName || '',
      avatar:      authUser.photoUrl    || '',
      // username and bio come from Social Firestore (fetched below)
      username:    '',
      bio:         '',
    };
  ...
  // Return the Wave custom token, Wave UID, email, and the 4 approved profile fields.
  // The Social UID, Social ID token, and Social password are NEVER returned.
  // socialProfile contains ONLY: displayName, avatar, username, bio.
  return new Response(JSON.stringify({
    customToken,
    waveUid,
...
async function handleRefreshSocialProfile(request, env, cors, sec) {
  ...
  try {
    ...
    if (fsRes.ok) {
      ...
      // 404 = Social profile doc does not exist yet — not a connectivity failure.
      // 403 = Firestore rules deny public read — Social rules may not be deployed.
      // Any other non-2xx = genuine connectivity/server error.
      // In all non-2xx cases we fall through and attempt the Auth-level fallback
      // rather than hard-failing, so the user still gets whatever fields are
      // available from Social Firebase Auth.
      const fsStatus = fsRes.status;
      if (fsStatus !== 404) {
        // Log the status for server-side diagnostics (no secrets logged)
        console.error(`[refresh-social-profile] Social Firestore returned HTTP ${fsStatus} for UID ${socialLocalId.slice(0,8)}...`);
      }
      // Non-fatal — fall through to Auth-level fallback below
    }
  } catch (e) {
    // Network error reaching Firestore — fall through to Auth-level fallback
    console.error('[refresh-social-profile] Social Firestore fetch error:', e.message);
  }

  // ── Auth-level fallback — fills displayName and avatar if Firestore didn't ───
  // This runs whether Firestore succeeded (to fill any missing fields) or failed.
  if (!refreshedProfile.displayName || !refreshedProfile.avatar) {
    try {
      ...
      if (authRes.ok) {
        ...
        // Auth lookup failed — this is a genuine connectivity/auth failure
        const authStatus = authRes.status;
        console.error(`[refresh-social-profile] Social Auth lookup returned HTTP ${authStatus} for UID ${socialLocalId.slice(0,8)}...`);
        // Only return the error if we also have no Firestore data
        if (!refreshedProfile.displayName && !refreshedProfile.avatar &&
            !refreshedProfile.username && !refreshedProfile.bio) {
      ...
      // Network error on Auth fallback
      console.error('[refresh-social-profile] Social Auth fetch error:', e.message);
      // Only fail hard if we have absolutely nothing to return
      if (!refreshedProfile.displayName && !refreshedProfile.avatar &&
          !refreshedProfile.username && !refreshedProfile.bio) {
        return new Response(JSON.stringify({ error: 'Could not reach Shadow Nexus Social profile.' }), {
          status: 502, headers: mergeHeaders(cors, sec, { 'Content-Type': 'application/json' })
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'POST' && url.pathname === '/upload-music') {
      ...
      const umClaims = await verifyFirebaseIdToken(idToken, WAVE_PROJECT_ID);
      if (!umClaims || umClaims.sub !== userUid) {
        return new Response(JSON.stringify({ error: 'Unauthorized — valid Firebase ID token required' }), {
          status: 401, headers: mergeHeaders(cors, sec, { 'Content-Type': 'application/json' })
        });
      }
