/**
 * Shadow Nexus Wave — firebase-live.js  v3
 *
 * Auth model
 * ───────────
 *  HOSTING  → requires a full Shadow Nexus Wave account (email/password).
...
export async function loadMyProfile() {
  ...
  try {
    ...
    return {
      uid,
      displayName: snxGetDisplayName(d, _auth.currentUser),
      username:    d.username || '',
      avatar:      d.avatar   || '',
...
export async function getUserProfile(uid) {
  try {
    ...
    return {
      uid,
      displayName: snxGetDisplayName(d, null),
      username:    d.username || '',
      avatar:      d.avatar   || '',
...
export async function joinRoom(roomId, name) {
  ...
  // For anonymous viewers, fall back to 'Viewer' only — they have no real identity
  await setDoc(doc(_db, 'liveUsers', uid), {
    userId:   uid,
    roomId,
    name:     displayName || (_auth.currentUser.isAnonymous ? 'Viewer' : 'User'),
    role:     'viewer',
    joinedAt: serverTimestamp(),
  });
}
...
export async function sendMessage(roomId, text) {
  ...
  // Resolve sender name: prefer Firestore profile (authoritative), fall back to liveUsers presence doc
  let senderName = '';
  let senderAvatar = '';
  try {
    // Try the full profile first — this is always the most accurate name
    if (!_auth.currentUser.isAnonymous) {
      const profileSnap = await getDoc(doc(_db, 'users', uid));
      if (profileSnap.exists()) {
        senderName   = snxGetDisplayName(profileSnap.data(), _auth.currentUser);
        senderAvatar = profileSnap.data().avatar || '';
      }
    }
    // If still empty (anonymous viewer), fall back to liveUsers presence record
    if (!senderName) {
      const liveSnap = await getDoc(doc(_db, 'liveUsers', uid));
      if (liveSnap.exists()) {
        senderName   = liveSnap.data().name   || '';
        senderAvatar = liveSnap.data().avatar || senderAvatar;
      }
    }
  } catch (_) {}
  if (!senderName) senderName = _auth.currentUser.isAnonymous ? 'Viewer' : 'User';
