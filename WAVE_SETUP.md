...
4. Under **Settings → Authorized domains**, add:
   ...
   - `www.shadownexussocial.online`

   > **⚠️ Fix for `auth/unauthorized-domain`**
   >
   > If users see the error `auth/unauthorized-domain` when logging in via
   > Shadow Nexus Social or Google, the browser's hostname is not in the list
   > above. The error fires inside `signInWithCustomToken`, `signInWithPopup`,
   > or `signInWithEmailAndPassword` when Firebase Auth rejects the current
   > page origin.
   >
   > **To fix:**
   > 1. Open: https://console.firebase.google.com/project/shadow-nexus-wave/authentication/settings
   > 2. Click **"Authorized domains"**
   > 3. Click **"Add domain"**
   > 4. Add the exact hostname shown in the browser address bar (no path, no port,
   >    no `https://` prefix — just e.g. `shadownexussocial.online`).
   > 5. Click **Save**.
   >
   > Only add domains that are under your control and genuinely host Wave.
   > Do **not** add `horr-a08f4.firebaseapp.com` or any Social Firebase domain here.
   > Do **not** add the Cloudflare Workers subdomain (`shadow-nexus-wave.nthntjrn.workers.dev`) —
   > that is the API backend, not a page host.
...
### ⚠️ Wave Worker Secrets (Set Manually)
...
# ── Social→Wave Authentication Bridge secrets ─────────────────────────────
# Required for POST /auth/social-bridge and POST /auth/refresh-social-profile.
# Without these, "Log in with Shadow Nexus Social" returns HTTP 503.

# SOCIAL_FIREBASE_WEB_API_KEY — Social Firebase Web API key (horr-a08f4 project).
# Value: AIzaSyByZRmp6R9HY17T2_WdJUFWeeaLNOP6y2Y
# This is the public client key — the same key the Social browser client uses.
# It is used server-side only to verify Social credentials against the
# Identity Toolkit REST API. It does NOT grant admin access to Social.
...
# Example: firebase-adminsdk-xxxxx@shadow-nexus-wave.iam.gserviceaccount.com
npx wrangler secret put WAVE_SA_CLIENT_EMAIL --config wrangler-wave.jsonc

# WAVE_SA_PRIVATE_KEY — Wave Firebase service account RSA private key (PKCS8 PEM).
# Get from: same JSON file, field "private_key".
# The value starts with "-----BEGIN PRIVATE KEY-----" and ends with
# "-----END PRIVATE KEY-----\n".
# Paste the full multi-line PEM value at the prompt — wrangler reads from
# stdin and handles newlines correctly.
...
Without `SOCIAL_FIREBASE_WEB_API_KEY` + `WAVE_SA_CLIENT_EMAIL` + `WAVE_SA_PRIVATE_KEY`: Social login returns 503.
...
| `SOCIAL_FIREBASE_WEB_API_KEY` | Social project key (`AIzaSyByZRmp6R9H...`) | ⚠️ Set if not already set |
| `WAVE_SA_CLIENT_EMAIL` | Wave service account email | ⚠️ Set if not already set |
| `WAVE_SA_PRIVATE_KEY` | Wave service account private key (PEM) | ⚠️ Set if not already set |
